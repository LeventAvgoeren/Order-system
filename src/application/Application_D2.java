...
public class Application_D2 {
   ...
   public <T> StringBuilder printStreamed(List<T> list, Function<Stream<T>, Stream<T>> streamManipulator) {
        ...
        .line()
        .row("Kund.-ID", "Name", "Kontakt")
        .line();


        TableFormatter tf2 = new TableFormatter(
            "| %10s ", "| %-32s", "| %36s |", "  %-20s  |"
        )
        .line()
        .row("Artikel-ID", "Beschreibung", "Preis CUR", " Mehrwertsteuersatz  ")
        .line();



        streamManipulator.apply(list.stream())
            .forEach(item -> {
                if (item instanceof Customer) {
                    ...
                    if(contacts.length > 1){
                        contact = contacts[0] +"(" + "+" + String.valueOf(contacts.length - 1)  + " contacts)";
                    } else {
                        contact = contacts[0];
                    }

                    tf.row(String.valueOf(customer.getId()), customer.getName(), contact);
                } else if (item instanceof Article){
                    Article article = (Article) item;
                    double price = (double) article.getUnitPrice()/100;

                    tf2.row(article.getId(), article.getDescription(), String.valueOf( price + " EUR"), String.valueOf(taxRateMapper.get(article.getTax()) + "% " + article.getTax()));
                }
            });
    ...
    void buildMore(List<Customer> customers, List<Article> articles, List<Order> orders) {
        ...
        var eric = find(customers, c -> c.getId()==892474L).get();
         var buch_Java = new Article("Buch \"Java\"", 4990)
                ...
                .setTax(TAX.GER_VAT_REDUCED);   // reduced tax rate on books
        //
        Article fahrradhelm= new Article("fahrradhelm", 16900).setId("SKU‐583978)").setTax(TAX.GER_VAT);
        Article fahrradkarte= new Article("fahrradkarte",695).setId("SKU‐583978").setTax(TAX.GER_VAT_REDUCED);
        Article pfanne= new Article("pfanne", 4999).setId("SKU‐300926").setTax(TAX.GER_VAT);
        Article tasse = new Article("Tasse", 299).setId("SKU-458362").setTax(TAX.GER_VAT);
        Article becher = new Article("Becher", 149).setId("SKU-693856").setTax(TAX.GER_VAT);
        Article kanne = new Article("Kanne", 1999).setId("SKU-518957").setTax(TAX.GER_VAT);
        articles.add(pfanne);
        articles.add(fahrradhelm);
        articles.add(fahrradkarte);
        
        Order ericsBestellung=new Order(eric)
        .setId("7372561535")
        .addItem(fahrradhelm, 1)
        .addItem(fahrradkarte, 1);

        orders.add(ericsBestellung);
        ...
        Order lenasOrder= new Order(lena)
        .addItem(buch_Java, 1)
        .addItem(fahrradkarte,1)
        .setId("6173535635");
        orders.add(lenasOrder);

        Order ericOrder2=new Order(eric)
        .addItem(tasse, 3)
        .addItem(kanne, 1)
        .addItem(becher, 3)
        .setId("4450735661");
        orders.add(ericOrder2);
   ...
   public StringBuilder printOrders(List<Order> orders) {
    ...
    //
    tf.row(null, null, null, null, "Gesamt:", fmtPrice(totals[1]), fmtPrice(totals[0]));
   ...
   public TableFormatter printOrder(Order order, TableFormatter tf) {
    ...
    tf.row(order.getId(), order.getCustomer().getFirstName() + "'s Bestellung:", null, null, null, "", "");



    for (OrderItem item : order.getItems()) {
        long price = calculateOrderItemValue(item);
        long vat = calculateOrderItemVAT(item);
        String vatSymbol = item.getArticle().getTax() == TAX.GER_VAT_REDUCED ? "*" : "";
        
       
        String ordered;
        if(item.getUnitsOrdered() > 1){
            ordered = ", " + item.getUnitsOrdered() + "x " + fmtPrice(price/item.getUnitsOrdered());
        } else {
            ordered = "";
        }
        tf.row("", "- " + item.getUnitsOrdered() + " " + item.getArticle().getDescription() + ordered, fmtPrice(vat), vatSymbol, fmtPrice(price) + " EUR", "", "");
    }

    tf.row("", null, null, null, null, fmtPrice(calculateOrderVAT(order)), fmtPrice(calculateOrderValue(order)) + " EUR");
   ...
   public long calculateOrderItemVAT(final OrderItem item) {
    ...
    public long calculateVAT(final long grossValue, final TAX tax) {
        if(grossValue<=0 ||tax==null){
            throw new IllegalArgumentException("Bitte gültige werte eingeben");
        }
        double steuren=taxRateMapper.get(tax);
        return Math.round(grossValue*steuren/100);
    }
